...
namespace FTServer.Monitor.SummaryImpl
{
    internal class SampleStream
    {
            ...
            for (var sampleIdx = 0; sampleIdx < samples.Count; sampleIdx++)
            {
                ...
                for (; i < _samples.Count; i++)
                {
                    ...
                    if (c.Value > sample.Value)
                    {
                        ...
                        _samples.Insert(i, new Sample { Value = sample.Value, Width = sample.Width, Delta = System.Math.Max(sample.Delta, System.Math.Floor(_invariant(this, r)) - 1) });
                        i++;
                        goto inserted;
                    }
                    r += c.Width;
        ...
        {
            var t = System.Math.Ceiling(q * N);
            t += System.Math.Ceiling(_invariant(this, t) / 2);
            var p = _samples[0];
            double r = 0;
